...
export function SingleItem({ track, onPlay, onAdd, className = '' }) {
  ...
  return (
    <div className={`flex flex-col gap-3 items-center w-full max-w-[22.5rem] ${className}`.trim()} aria-label={`Single ${t.title}`}>
...
export default function Singles({ tracks = [], onPlay, onAdd }) {
  ...
  return (
    <section 
      ...
      aria-label="Singles"
    >
      <h2 className={`font-cal-sans font-bold text-[1.75rem] m-0 mb-2 ${isMobile ? 'px-4' : ''}`}>Singles</h2>
